...
function ProductForm({ initial, onSave, saving }: { initial: ProductBody; onSave: (v: ProductBody) => void; saving: boolean }) {
  const api = useAdminApi(); const [form, setForm] = useState(initial); const [prevInitial, setPrevInitial] = useState(initial); const [categories, setCategories] = useState<AdminCategory[]>([]); const [error, setError] = useState("");
  if (initial !== prevInitial) {
    setPrevInitial(initial);
    setForm(initial);
  }
  useEffect(() => { runAfterCommit(() => { void api.listCategories().then(setCategories).catch((e) => setError(errorMessage(e))); }); }, []); // eslint-disable-line react-hooks/exhaustive-deps
...
export function ProductEditorPage() {
  ...
  useEffect(() => { runAfterCommit(() => load()); }, [id]); // eslint-disable-line react-hooks/exhaustive-deps
